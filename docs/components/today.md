...
| `readiness` | 下一节课教材和携带物清单 | `1x1` |
| `course-tasks` | 课程任务与未完成状态 | `1x1` |
| `plans` | 今日计划，复用 `dayPlans` | `1x1` |
| `week-glance` | 当前周、今日课程和完成摘要 | `1x1` |
| `weather` | 当前天气、逐时温度和动态图标 | `2x2` |

`size` 支持旧别名 `compact / standard / large`，也支持明确网格尺寸 `1x1 / 1x2 / 2x1 / 2x2`。

默认布局以 `2x1` 的课程摘要、唯一一张 `2x2` 天气主视觉和成对的 `1x1` 快速信息卡组成。尺寸变化不仅会调整外框，也会改变信息层级；内置“本周一览”和天气会根据可用空间逐步展示内容，自定义模块可以通过插槽作用域采用同样的响应式策略。
...
| `2x2` | 完整指标、图表和补充结论 | 逐时天气、七日课程柱状图与周总结 |
