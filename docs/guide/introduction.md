...
- **分层架构**:
  - `@yotsuba/schedule-core` — 零依赖 TypeScript,纯函数引擎 + 协议定义,可独立用于任何框架;
  - `@yotsuba/schedule-vue` — Vue 3 组件 `<YsSchedule>`;
  - `@yotsuba/schedule-elements` — `<ys-schedule>` 自定义元素,React / uni-app / 原生页面直接可用。
